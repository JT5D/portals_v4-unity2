...
- **Release builds embed JS bundle**: When building with `-configuration Release`, the JavaScript bundle is embedded at build time. Metro hot reload does NOT work - you must reinstall the app to see TypeScript/JS changes. This is why `react-native-unity` requires Release mode (DEBUG has `_mh_dylib_header` linker bug).
...
### 6. Unity iOS Build Architecture (KEY INSIGHTS)
...
### 7. Fabric/New Architecture Component Registration (CRITICAL FIX)
...
**How to diagnose**: Add file-based logging to `RNUnityView.mm` (see `node_modules/@artmajeur/react-native-unity/ios/RNUnityView.mm`) and check `Documents/unity_init.log` on device.

**Note**: This fix may need to be reapplied after `pod install` or when generated files are regenerated. Consider adding a post-install script to automate this.
