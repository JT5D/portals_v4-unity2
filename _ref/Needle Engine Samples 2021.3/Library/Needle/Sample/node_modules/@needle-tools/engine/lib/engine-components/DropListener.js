...
export class DropListener extends Behaviour {
    ...
    onDrag = (evt) => {
        if (debug)
            console.debug("DropListener Drag", evt, this.context.connection.allowEditing);
        if (this.context.connection.allowEditing === false)
            return;
    ...
    onDrop = async (evt) => {
        if (debug)
            console.debug("DropListener Drop", evt, this.context.connection.allowEditing);
        if (this.context.connection.allowEditing === false)
            return;
        if (!evt?.dataTransfer)
            return;
    ...
    testIfIsInDropArea(ctx) {
        ...
        const hits = this.context.physics.raycast({
            ...
            testObject: obj => {
                ...
                if (this._addedObjects.some(o => o.contains(obj)))
                    return false;
                return true;
            }
        });
