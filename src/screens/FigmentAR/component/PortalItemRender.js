...
var PortalItemRender = createReactClass({
  ...
  componentDidMount() {
    ...
    this.setTimeout(() => {
      if (this._isMounted) {
        console.log('[PortalItemRender] Forcing re-render for UUID:', this.props.portalIDProps.uuid);
        this.forceUpdate();
      }
    }, 100);
  },
  ...
  componentWillUnmount() {
    console.log('[PortalItemRender] componentWillUnmount - UUID:', this.props.portalIDProps.uuid);
  ...
  _updateInitialRotation() {
    ...
    if (!this.arNodeRef) {
      console.log('[PortalItemRender] _updateInitialRotation - No ref, making visible anyway');
      this.setState({
        shouldBillboard: false,
        nodeIsVisible: true,
      });
    ...
    this.arNodeRef.getTransformAsync().then((retDict) => {
      ...
      console.warn('[PortalItemRender] _updateInitialRotation error:', error);
    });
  },
});
